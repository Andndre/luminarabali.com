...
<div x-data="thumbnailGenerator()" class="mb-6 flex justify-between items-center">
    ...
    <div class="flex items-center gap-3">
        ...
        <a href="{{ route('admin.component-library.create') }}" class="px-4 py-2 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 transition flex items-center gap-2 shadow-sm">
...
<!-- Off-screen area for thumbnail rendering -->
<div style="position: fixed; top: -9999px; left: -9999px; z-index: -9999; pointer-events: none;">
    <div id="render-target" style="width: 800px; height: 450px; background-color: white; position: relative;"></div>
</div>

<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
    @forelse($components as $component)
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col hover:border-indigo-300 transition-colors">
        <!-- Thumbnail -->
        <div class="aspect-video bg-white border-b border-gray-200 relative overflow-hidden group">
            @if($component->thumbnail)
                <img src="{{ asset($component->thumbnail) }}" alt="{{ $component->name }}" class="w-full h-full object-contain p-4">
            @else
            ...
            <div class="absolute top-2 left-2 flex gap-1">
                <span class="px-2 py-1 text-[10px] uppercase tracking-wider font-semibold rounded shadow-sm border border-gray-200 {{ $component->type === 'section' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-white text-gray-700' }}">
                    {{ ucfirst($component->type) }}
                </span>
                <span class="px-2 py-1 text-[10px] uppercase tracking-wider font-semibold rounded shadow-sm border border-gray-200 {{ $component->is_public ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-white text-gray-500' }}">
            ...
            <div class="absolute top-2 right-2">
                <span class="px-2 py-1 text-[10px] uppercase tracking-wider font-semibold bg-white/90 backdrop-blur text-gray-600 rounded shadow-sm border border-gray-200">
            ...
            <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <a href="{{ route('admin.component-library.edit', $component->id) }}" class="p-2 bg-indigo-600 text-white rounded-lg shadow-lg hover:bg-indigo-700 transition-transform hover:scale-105" title="Edit">
                ...
                <form action="{{ route('admin.component-library.destroy', $component->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this component?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="p-2 bg-red-600 text-white rounded-lg shadow-lg hover:bg-red-700 transition-transform hover:scale-105" title="Delete">
        ...
        <div class="p-4 flex-1 flex flex-col bg-white">
            ...
            <p class="text-[11px] text-indigo-600 font-mono-code mb-2 truncate" title="{{ $component->slug }}">{{ $component->slug }}</p>
            <p class="text-sm text-gray-500 line-clamp-2 mt-auto">{{ $component->description ?: 'No description provided.' }}</p>
    ...
    Alpine.data('thumbnailGenerator', () => ({
        ...
        async generateAll() {
            ...
            for (const component of this.components) {
                ...
                // Render inside an inline-block wrapper with tiny padding, transparent background, and forced Plus Jakarta Sans font metrics
                target.innerHTML = `<div id="render-content" class="font-sans" style="display: inline-block; padding: 4px; background: transparent; vertical-align: top; font-family: 'Plus Jakarta Sans', sans-serif;">` +
                    `${html}` +
                    `</div>`;
                
                // Wait for Tailwind to process and fonts to be ready
                await document.fonts.ready;
                await new Promise(r => setTimeout(r, 800));
                
                try {
                    const contentElement = target.querySelector('#render-content') || target;
                    const blob = await window.htmlToImage.toBlob(contentElement, {
                        pixelRatio: 2,
                        skipFonts: false
                    });
